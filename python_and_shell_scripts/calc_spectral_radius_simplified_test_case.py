...
for i in range(M*fperc):
    ...
    if(this_f == fperc):
        ...
        M_1[i][(this_m-1)*fperc] += (this_mu/(sigT*h_k) + 0.5*(1 + alfa))
        
        M_2[i][i] += 0.5*(1 - alfa)*this_wt
        M_2[i][(this_m-1)*fperc] += 0.5*(1 + alfa)*this_wt
...
for a in range(fperc):
    for b in range(fperc):
        for c in range(fperc):
            K[a][b] -= 0.5/float(fperc)*(M2_M1i_sum[a][c])
        K[a][b] += 0.5*M2_M1i_sum[a][b]
...
for h in range(fperc):
    if math.fabs(eigs.item(h).real) > max_eig:
        ...
        max_f = h+1
    print '{0:7.5f}'.format(eigs.item(h))

print "\n"        
print "Hist. spectral radius = " + str(max_eig)
print "   ...at lambda = " + str(max_lambda)
print "    ...and cell = " + str(max_f)
            
#####################################################################################################
##BEGIN EXPONENTIAL METHOD....#######################################################################      
    
max_eig = 0.0
lambda_range_f = npy.arange( (2*math.pi)/(sigT*X), (fperc-1)*(2*math.pi)/(sigT*X)+0.001, (2*math.pi)/(sigT*X)) 
for this_lambda_f in lambda_range_f:
    wt_sum = 0.0
    for m in range(M):  
        this_mu = mus.item(math.floor(float(m)/2))*math.pow(-1.0,m+1) 
        this_wt = wts.item(math.floor(float(m)/2)) 
        alfa_hat = 0.5*(1+alfas[m])*complex(math.cos(sigT*this_lambda_f*0.5*h_k), math.sin(sigT*this_lambda_f*0.5*h_k)) + 0.5*(1-alfas[m])*complex(math.cos(-sigT*this_lambda_f*0.5*h_k), math.sin(-sigT*this_lambda_f*0.5*h_k))
        wt_sum += this_wt/((this_mu/(sigT*h_k*alfa_hat))*(complex(math.cos(sigT*this_lambda_f*0.5*h_k), math.sin(sigT*this_lambda_f*0.5*h_k)) - complex(math.cos(-sigT*this_lambda_f*0.5*h_k), math.sin(-sigT*this_lambda_f*0.5*h_k))) + 1)
        
    this_eig = 0.5 * wt_sum
    
    if(math.fabs(this_eig.real) > max_eig):
        max_eig = math.fabs(this_eig.real)     

    print '{0:7.5f}'.format(this_eig)
    
print "\n"        
print "Expo. spectral radius = " + str(max_eig)
print "   ...at lambda = " + str(max_lambda)
print "    ...and cell = " + str(max_f)  
